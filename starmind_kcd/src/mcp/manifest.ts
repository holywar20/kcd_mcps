 ...
 * ══ VENDORED ═════════════════════════════════════════════════════════════════
 *
 * Copied verbatim from `kcd_sdk/src/server/manifest.ts` on 2026-07-22, during the
 * Daedalus extraction. A deliberate copy — see `./McpServer.ts`'s header for the
 * full rationale and the divergence contract.
 *
 * WHY THE LIFECYCLE FIELDS SURVIVE. `installed`, `exposed`, and `entryPoint` read
 * like Starmind plugin machinery that a narrowly-scoped context tool should shed.
 * They are kept ON PURPOSE ( Bryan, 2026-07-22 ): they are the interop contract
 * that lets Starmind arm and disarm this server exactly the way it arms and disarms
 * every other one. Trimming them would buy a slightly purer type and cost a second
 * manifest shape for the host to learn — a bad trade for a server the host already
 * knows how to manage.
 *
 * The fields are therefore load-bearing for INTEROP even where Daedalus itself does
 * not read them. Daedalus's own installer never consults them.
 *
 * ═════════════════════════════════════════════════════════════════════════════
 */
